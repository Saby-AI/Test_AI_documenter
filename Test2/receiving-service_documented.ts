...
class ReceivingService {
  ...
  async executeLoadOutLoadIn(loadOutLoadinQry: string, lnQcount: number, lnWcount: number, fscanentime: Date): Promise<void> {
    try {
      await this.manager().query(`BEGIN ${loadOutLoadinQry}; END;`)
        .catch(error => {
          this.logger.error({ lnQcount, lnWcount, fscanentime }, 'Error in LOADOUT/LOADIN UPDATE Query', 'RECEIVING > PROCESS_OUTCLOSE');
          throw error;
        });
    } catch (error) {
      this.logger.error({ error, message: 'LOADIN error OUTCLOSE -->' }, 'Error in OUTCLOSE', ReceivingService.name);
    }
    this.logger.debug({ service: ReceivingService.name, curOper: recvo.curOper }, `receiving --> OUTCLOSE | Elapsed time ${moment().diff(startTime)} ms | OUT Time ${moment().format('HH:mm:ss-SSS')}`);
  }
  /**
  ...
  async SEEIFXDOCK(recvo: ReceivingVO): Promise<void> {
    if (recvo.lc_batch && recvo.lc_batch.length > 0 && recvo.LOADIN && recvo.LOADIN?.fhasxdock !== true) {
      ...
      const HASXDOCKResult = await this.manager().query(`BEGIN SELECT TOP 1 id FROM dbo.INV_MST WHERE fcanxdock = 1 and fproduct = '${recvo.lc_prod}' and fcustcode = '${recvo.lc_CustCode}' order by fbatch ASC ; END`);
      const HASXDOCK: InvMst = HASXDOCKResult[0];
      if (HASXDOCK) {
        // Querying LOADIN details based on batch
        const LOADINResult = await this.manager().query(`BEGIN SELECT TOP 1 * FROM dbo.Loadin WHERE fbatch = '${recvo.lc_batch.padStart(7, '0')}' order by fbatch ASC; END`);
        const LOADIN: Loadin = LOADINResult[0];
        // Update the LOADIN if it exists
        if (LOADIN) {
          LOADIN.fhasxdock = true;
          await this.manager().query(`BEGIN UPDATE Loadin set fhasxdock = 1 WHERE fbatch = '${recvo.lc_batch.padStart(7, '0')}'; END`);
   ...
   * @returns {Promise<void>}
   */
  async WRITEINVCONTROL( fwho: string, recvo: ReceivingVO, fproblem: string, fhowfixed: string, fresolved: boolean, ): Promise<void> {
    const fwhen: string = fresolved ? `CAST([dbo].[Localdatetime](sysdatetime()) AS datetime2)` : `Null`;
    await this.facilityService.getConnection().createEntityManager().query(`BEGIN INSERT into INVCONTROL ( foperid,fdatestamp,fworktype,fbatch,fpalletid, fproblem,fhowfixed, fcustcode, fwho, fwhen, fresolved ) VALUES ( '${fwho}', CAST([dbo].[Localdatetime](sysdatetime()) AS datetime2), 'RFDATECHEC', '${recvo.lc_batch}', '${recvo.lc_pal}', '${fproblem}', '${fhowfixed}', '${recvo.lc_CustCode}', '${fresolved ? fwho : ''}', ${fwhen}, ${fresolved ? 1 : 0} ); END;`);
  }
...
🎯 GitHub Operation Completed Successfully!
📁 Repository: Saby-AI/Test_AI_documenter
🌿 Branch: main
📄 Documented file: receiving-service_documented.ts
📊 Analysis file: receiving-service_analysis_ts.md
🔄 Operation type: document
✨ No local files created (GitHub-only mode)
=== NEXT STEP ===
                To commit this documented code to GitHub, run:
                operation='document_commit'
                file_path='Test2/receiving-service.ts'
